...
async function multihashSha256(input: Uint8Array): Promise<Uint8Array> {
  ...
  const inputBuffer = new Uint8Array(input).buffer as ArrayBuffer
...
function encodeDocument(document: Document): string {
  return toMultibaseB58(toMulticodecJson(document))
}
...
export async function decode(did: string): Promise<Document> {
  ...
  return fromMulticodecJson(fromMultibaseB58(doc))
}

function operateOnEmbedded(callback: (document: Document) => Document): any {
  return function (document: Document | string): Document | string {
    if (typeof document === "string") return document
    return callback(document)
  }
}

function visitVerificationMethods(document: Document, callback: (document: Document) => Document): Document {
...
function contextualizeDocument(did: string, document: Document): Document {
  ...
  visitVerificationMethods(contextualized, (vm) => {
    if (vm.controller === undefined) vm.controller = did
    return vm
  })
