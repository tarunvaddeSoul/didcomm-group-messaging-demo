...
import { toBase64Url, fromBase64Url } from "../protocol/crypto"
...
export default class DIDPeer {
  static keyToMultibase(key: Uint8Array, prefix: multicodec.CodecName): string {
    ...
    // Avoid String.fromCharCode(...buffer) which overflows the stack for large arrays
    let result = ""
    ...
    elements.forEach(element => {
      ...
      switch (purposeCode) {
        case purposeCodeList["Verification"]: {
          const decodedSigningKey = multicodec.rmPrefix(multibase.decode(encodedValue))
        ...
        case purposeCodeList["Encryption"]: {
          const decodedEncryptionKey = multicodec.rmPrefix(multibase.decode(encodedValue))
        ...
        case purposeCodeList["Service"]: {
          ...
          services = services
            ...
            .filter((service: any) => service.type == "DIDCommMessaging")
          if (!Array.isArray(doc.service)) doc.service = []
